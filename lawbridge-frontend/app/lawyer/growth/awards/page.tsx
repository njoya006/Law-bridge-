...
export default function AwardsPage() {
  ...
  const achievements: Achievement[] = [
    { id: 'verified',   label: 'Verified Professional', desc: 'Completed identity and credential verification',            icon: '✅', unlocked: !!profile?.verified_at,   tier: 'gold' },
    { id: 'pioneer',    label: 'Pioneer',               desc: 'One of the first lawyers on LawBridge',                    icon: '🚀', unlocked: true,                     tier: 'silver' },
    { id: 'closer5',    label: 'Case Closer',           desc: 'Resolved 5+ cases successfully',                           icon: '⚖️', unlocked: closedCases >= 5,         tier: 'bronze' },
    { id: 'closer25',   label: 'Veteran Litigator',     desc: 'Resolved 25+ cases',                                       icon: '🏛️', unlocked: closedCases >= 25,        tier: 'silver' },
    { id: 'closer100',  label: 'Elite Counsel',         desc: 'Resolved 100+ cases',                                      icon: '👑', unlocked: closedCases >= 100,       tier: 'gold' },
    { id: 'toprated',   label: 'Top Rated',             desc: 'Maintained a 4.5+ average client rating',                  icon: '⭐', unlocked: rating >= 4.5,           tier: 'gold' },
    { id: 'reputation', label: 'Rising Star',           desc: 'Achieved a reputation score above 50',                     icon: '🌟', unlocked: score >= 50,             tier: 'silver' },
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      {!loading && profile && (
        <div className="rounded-2xl border border-gold-400/20 bg-gold-500/[0.04] p-6 flex items-center gap-5">
          <div className="flex-shrink-0 w-16 h-16 rounded-full bg-gradient-to-br from-gold-500/30 to-gold-700/20 border border-gold-400/30 flex items-center justify-center text-2xl">
            🏆
          </div>
          <div>
      ...
      {loading && <div className="h-28 rounded-2xl border border-white/8 bg-primary-800/20 animate-pulse"/>}
      ...
      {unlocked.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {unlocked.map(a => (
              <div key={a.id} className={`rounded-2xl border p-4 flex items-center gap-4 ${a.tier ? tierColors[a.tier] : 'border-white/8 bg-primary-800/20'}`}>
      ...
      {locked.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {locked.map(a => (
              <div key={a.id} className="rounded-2xl border border-white/5 bg-primary-900/30 p-4 flex items-center gap-4 opacity-50">
