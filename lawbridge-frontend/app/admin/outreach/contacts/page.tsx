    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <form onSubmit={submit} onClick={e => e.stopPropagation()} className="w-full max-w-lg rounded-2xl bg-primary-800 border border-white/10 p-6 space-y-4">
        ...
        <div className="flex justify-end gap-3">
          ...
          <button type="submit" className="rounded-xl bg-gold-500 px-5 py-2 text-sm font-semibold text-primary-900 hover:bg-gold-400">Add Contact</button>
        </div>
      </form>
    </div>
  )
          ...
          <tbody className="divide-y divide-white/5">
            {filtered.length === 0 ? (
              ...
              <tr key={c.id} className="hover:bg-white/3 transition-colors">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-1.5">
                        ...
                        {c.isPrimary && <span className="text-[10px] rounded-full bg-gold-500/15 text-gold-400 px-1.5 py-0.5">Primary</span>}
                      </div>
                ...
                <td className="px-4 py-3">
                  <Link href={`/admin/outreach/firms/${c.firmId}`} className="text-sm text-neutral-400 hover:text-gold-400 transition-colors">{c.firmName}</Link>
                </td>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1 justify-end">
                    ...
                    <button onClick={() => handleDelete(c.id)} className="rounded-lg p-1.5 text-neutral-500 hover:text-red-400 hover:bg-red-500/10 transition-colors">
