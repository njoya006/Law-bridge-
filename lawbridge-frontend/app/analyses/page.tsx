    ...
    <div>
      ...
      <div className="mt-4 space-y-3">
        {items.length===0 && !error && (
          <EmptyState icon={<AnalysisIcon width={24} height={24} />} title="No updates yet" body="Live progress on your matters will appear here." />
        )}
        {items.map((a,i)=> (<div key={a.id} className="p-3 bg-primary-800 rounded stagger-child" style={{ '--i': Math.min(i, 8) } as React.CSSProperties}><Link href={`/analysis/${a.id}`} className="text-white">{a.case_type} — {a.status}</Link></div>))}
      </div>
    </div>
  )
}
