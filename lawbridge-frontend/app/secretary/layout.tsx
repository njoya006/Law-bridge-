...
const nav = [
  ...
  { label: '📊 Reports', href: '/secretary/reports' },
  { label: '👥 Members', href: '/secretary/members' },
]
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-40 w-60 flex flex-col bg-primary-900 border-r border-neutral-700/40">
      <div className="flex items-center gap-3 px-5 py-5 border-b border-neutral-700/30">
        <div className="w-9 h-9 rounded-xl bg-gold-500/20 border border-gold-500/30 flex items-center justify-center text-gold-400 font-bold text-sm flex-shrink-0">{initials}</div>
      ...
      <div className="px-3 py-4 border-t border-neutral-700/30 space-y-1">
        <Link href="/lawyer/office/me" className="flex items-center px-3 py-2 rounded-lg text-xs text-neutral-500 hover:text-neutral-300 hover:bg-primary-800/40 transition-colors">
          ← Lawyer Office
        </Link>
        <button
          onClick={() => { clearSession(); router.push('/auth/login') }}
          className="flex items-center px-3 py-2.5 rounded-lg text-sm font-medium text-neutral-400 hover:text-red-300 hover:bg-red-900/20 transition-colors w-full"
        >
          Sign out
        </button>
      </div>
