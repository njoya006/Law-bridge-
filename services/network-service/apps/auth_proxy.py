...
class AuthProxyAuthentication(BaseAuthentication):
    def authenticate(self, request):
        ...
        # Identify by user_id (always present, incl. refreshed tokens) and fall back
        # to email. SimpleJWT drops custom claims like `email` from refreshed access
        # tokens, so requiring email broke follow/feed/referrals after a token refresh.
